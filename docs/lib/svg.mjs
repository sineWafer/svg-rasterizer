 ...
 */
export function applyAnimation(svg, skipAnimationElements = true) {
  const svgWindow = svg.ownerDocument.defaultView;
  if (svgWindow === null) return;

  // The only animatable attribute that can be on the <svg> itself we care about is view box. Everything else has to
  // do with the size or position of the SVG, which we override anyway.
  {
    const vb = svg.viewBox.animVal;
    if (vb !== null) {
  ...
  for (const element of (/** @type {NodeListOf<SVGElement>} */ (svg.querySelectorAll('*')))) {
    ...
    for (let i = 0; i < computedStyle.length; ++i) {
      ...
      if (attributeValue === defaults.getPropertyValue(attributeName)) {
        continue;
      }
 ...
 */
export function getAllAnimationElements(svg) {
