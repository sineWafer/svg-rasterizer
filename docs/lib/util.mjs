 ...
 */
export function parsePositiveInt(value, allowZero) {
 ...
 */
export function toDisplayTimeDelta(seconds) {
  const minutes = Math.floor(seconds / 60);
  seconds -= 60 * minutes;
  const secondsFloored = Math.floor(seconds);
  const ms = Math.floor(1000 * (seconds - secondsFloored));
  seconds = secondsFloored;
 ...
 */
export function parseComputedLength(computedValue) {
  const match = /(\d+(?:\.\d+)?)\s*px/.exec(computedValue);
